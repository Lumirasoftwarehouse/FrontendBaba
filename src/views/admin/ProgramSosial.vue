    ...
    async changeStatus(id, status) {
      try {
        const confirmResult = await Swal.fire({
          ...
          text: `Anda akan ${
            status == "1" ? "mengaktifkan" : "menonaktifkan"
          } program sosial ini.`,
          icon: "warning",
        ...
        if (confirmResult.isConfirmed) {
          ...
          Swal.fire({
            icon: "success",
            title: "Request Berhasil",
            text: `Program sosial berhasil ${
              status == "1" ? "diaktifkan" : "dinonaktifkan"
            }`,
          });
          this.fetchDataSosial();
        } else {
          Swal.fire({
            ...
            text: "Perubahan status dibatalkan.",
          });
        }
      } catch (error) {
        console.error(error); // Handle error if any
